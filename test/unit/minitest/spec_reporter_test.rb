...
module MinitestReportersTest
  class SpecReporterTest < Minitest::Test
    ...
    def test_spec_reporter_with_option_show_order_after
      ...
      @test.name = "test_0023_should_work_with_show_order_after"
      @test.time = 0.1
      assert_output(/^  test_0023_should_work_with_show_order_after \s* PASS \(\d\.\d\ds\)\n$/) do
        reporter.io = $stdout
        reporter.record(@test)
      end
    end
    ...
    def test_spec_reporter_with_option_show_order_before
      ...
      @test.name = "test_0099_should_work_with_show_order_before"
      @test.time = 0.1
      assert_output(/^ PASS  should_work_with_show_order_before  -  \(\d\.\d\ds\)\n$/) do
        reporter.io = $stdout
        reporter.record(@test)
      end
      @test.name = "test_: Should work with show order before"
      @test.time = 0.1
      assert_output(/^ PASS  Should work with show order before  -  \(\d\.\d\ds\)\n$/) do
        reporter.io = $stdout
        reporter.record(@test)
      end
    end
